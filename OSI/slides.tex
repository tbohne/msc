...
\begin{frame}
  ...
  \begin{itemize}
    \item \textbf{Simulation:} Auslösbar durch Nachricht auf jeweiligem Topic, z.B. \code{rostopic pub -1 /sim\_docking\_failure\_raised\_ramp std\_msgs/String fail}
    \item \textbf{Monitoring:} Erkennt Problem + unterbricht \code{NORMAL\_OPERATION} + initiiert geeignete Lösung / Kommunikation
    \item \textbf{Lösung:}
    \begin{itemize}
      \item Löst das Problem + Übergang zu \code{NORMAL\_OPERATION}
      \item Löst das Problem nicht + Übergang zu \code{CATASTROPHE}
    \end{itemize}
  \end{itemize}
\end{frame}
