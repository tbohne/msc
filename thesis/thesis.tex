...
is a path to the navigation goal, but \code{move\_base\_flex} cannot find it (cf. fig. \ref{fig:nav_fail}).
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{pics/nav_fail.png}
    \caption{\textsc{Navigation Failure: \code{move\_base\_flex} is unable to find a path to the goal.}}
    \label{fig:nav_fail}
\end{figure}
...
and that should be detected by monitoring. Furthermore, the following (cf. fig. \ref{fig:obstacle_scenarios}) solvable static obstacle scenarios were implemented in order to test the robot's ability to overcome them.
\begin{figure}[H]
    \centering
    \begin{subfigure}[b]{0.24\textwidth}
        \centering
        \includegraphics[width=\textwidth]{pics/nav_fail.png}
        \caption{\textsc{}}
        \label{fig:prototypical_layout}
    \end{subfigure}
    \hfill
...
\end{figure}
In this manner, the situation that a static obstacle appears on the robot's route can be simulated, and the robot is usually able to manage it (cf. fig. \ref{fig:obstacle_success}).
...
\begin{figure}[H]
    ...
    \begin{subfigure}[b]{0.24\textwidth}
        ...
        \label{fig:example_scan_route}
    \end{subfigure}
\caption{\textsc{Successfully Dealing with Static Obstacles.}}
\label{fig:obstacle_success}
\end{figure}
The models used must be stored in \code{~/.gazebo/models} and are all freely available \footnote{http://models.gazebosim.org/}. Unfortunately, it is not trivial to
...
simulate such situations works as follows. The user can specify a set of points, e.g., the one shown in fig. \ref{fig:outlier_points} in the scenario described in section \ref{sec:prototype_scenario}.
...
When simulating such a situation, the point farthest from the robot's current position is chosen, as this will most likely cause navigation problems.
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{pics/nav_fail.png}
...
\end{figure}
As anticipated, this is not a guarantee of navigation failure, but it does cause it on a regular basis. It would be infeasible to simply interrupt the active plan and set the
selected point as navigation target, as there would then be no successful outcome that continues the plan. Thus, the dummy target is introduced as an intermediate action in the
plan (an option implemented as part of the low-level operation state machine).
