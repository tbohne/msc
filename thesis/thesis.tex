...
Practically relevant challenges for LTA identified, classified in terms of potential impact and implemented in simulation

Generic monitoring (resolution) framework developed and evaluated for reliability.

All problem types have been moved to category (1) / (2) - The goal was to detect all problem types at least reliably and, if possible, also to solve them directly

Significantly improved robustness with regard to the identified challenges (Reliability at least in the simulation was experimentally substantiated

before the system was not able to handle these problems at all)
...
Field tests with the real AROX system, after it performs well in simulation, the next step would be to demonstrate it on the real platform.

Further experiments (seed, runtime, error frequency, etc.)
Then further experiments, I have now initialized the 10 runs always with the same seed, so also always the same sequence of problems that are simulated, to have some comparability between the runs.
in the long term, however, you would naturally want to test the entire set
then runtime and error frequency can of course also vary and provide interesting insights

Using database entries: Learning from LTA Missions (as described in section \ref{})

More elaborate solution methods (focus here detection)
then more elaborate solution methods would be interesting, the focus in my work is on the monitoring and I have always implemented only simple workarounds, there is of course still great potential.

Web interface: remote access to sensors / control
It would be nice if human operators had remote access to the robot and could simply access the sensors in the event of minor problems or control the robot remotely, things like that.
or in the simplest case an improvement of the communication with the operator, which I implemented in pure text form and there one could imagine of course graphically prepared sensor, weather and further information over a web interface well

Usability: Integration / reconfiguration of the framework
I have always indicated that I put a lot of emphasis on generality and generic solutions, so one of my next goals would be to improve and simplify the integration of the monitoring framework and reconfigure certain aspects of it.
