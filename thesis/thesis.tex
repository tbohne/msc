...
\end{itemize}
Navigation failures that are in principle solvable, i.e. not inevitable, can be addressed with the resolver described in section \ref{sec:nav_fail_resolver}.
...
and local costmaps are cleared. The order is crucial - first, the global costmap is cleared using the \code{move\_base\_flex/clear\_costmaps} service. Subsequently, the local costmap
is cleared, which is implemented as a recovery, i.e. a \code{SimpleActionClient(move\_base\_flex/recovery, RecoveryAction)} is created and a \code{`clear\_costmap'} goal is sent.
...
executed: Driving the robot to one of several specified recovery points. Since these points are set by the user with the intention of being easily accessible from almost any location
possible in the scenario, it often frees the robot from deadlocked situations and allows the mission to continue from there. Obvioulsy, it is not possible to solve inescapable
