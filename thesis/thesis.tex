...
monitoring is to check expected values for known states. All localization monitoring is performed at a frequency specified by the user.
...
the robot's current pose (position + orientation). The position is taken from \code{NavSatFix} messages to the \code{/fix} topic, and the angular rotation, i.e. orientation, 
based on the IMU's gyroscope is extracted from the latest \code{Imu} message to \code{/imu\_data}.
...
created, which then accounts for the actual $180\degree$ rotation. Such a problem can occur in practice in various situations, e.g. when the robot unexpectedly slips away and turns.
