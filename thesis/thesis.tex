...
in the basic configuration assumed in this work does not have an explicit knowledge representation, but only a predefined static map of the environment. Moreover, the map
is not updated, i.e. no learning takes place in this sense, and no environmental changes are taken into account. In this simple scenario, reasoning is also disregarded. Another
interesting point is perception. There is a kind of perception in the form of obstacle detection based on laser scans, but it is not a particularly elaborate form of perception.
Even less part of the system is learning, technically this aspect also occurs somehow, e.g. in the sense that obstacles are detected and entered into the local costmap. Of course,
...
understood by machine learning. A counterexample is interaction, which is again very explicitly part of the scenario under consideration, namely in the form of communication and
cooperation with the human operator (cf. \ref{sec:fallback_solution}). The planning process itself is not part of the scenario, but it is assumed that plans are available and usable,
...
used to report explicit navigation errors.\newline
