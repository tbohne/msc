...
\code{NORMAL\_OPERATION} is represented by an embedded state machine named \code{OPERATION} that is visualized in figure \ref{fig:low_level_smach} as well as the two
parallel running execution monitoring states \code{CONTINGENCY\_MONITORING} and \code{CATASTROPHE\_MONITORING}.
\begin{figure}[H]
    \centering
...
During the state \code{OPERATION}, i.e. in the nested state machine visualized in figure \ref{fig:low_level_smach}, the robot can be either in the state \code{IDLE}, in which it is waiting for a plan, 
or in \code{EXECUTE\_PLAN}, in which it is executing a given plan. The embedded state machine \code{OPERATION} has four possible outcomes - \code{end\_of\_episode},
\code{minor\_complication}, \code{critical\_complication}, and \code{preempted}. It starts in the \code{IDLE} state and remains there as long as no plan is provided via a ROS service.
...
are capable of interrupting the \code{IDLE} state at any time due to external problems leading to the outcome \code{preempted}.
...
Now that we have seen what happens in \code{NORMAL\_OPERATION} and particularly in \code{OPERATION}, we come back to the parent state machine depicted in figure \ref{fig:high_level_smach}.
...
\code{minor\_complication}, the robot enters the state \code{CONTINGENCY}, which means that it has detected a minor problem, e.g. a sensor failure, but its ability to act
is preserved, it is for example able to drive back to the base in safety mode. However, if \code{NORMAL\_OPERATION} returns \code{critical\_complication}, there is a critical 
problem that is unsolvable for the robot, perhaps even for the operator, and the robot transitions to \code{CATASTROPHE}. If it is still possible, the robot saves the current 
