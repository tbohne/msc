...
recovery count reaches a user-configurable recovery limit $l \in \mathbb{N}$, i.e. $r \geq l$, a contingency is initiated due to a sustained recovery. This method covers a sustained recovery, as it
arises from a case of obstacles without the possibility of circumvention. Combined with a check for explicit \code{move\_base\_flex} errors, this covers all navigation failures, since
they will all end with a persistent recovery or an explicit termination. One problem is that repeated recoveries in which some progress is actually achieved should not be mistaken as
...
failure. In such cases, the recovery counter is reset, i.e. $r = 1$, the initial recovery position is set to the present position and the recovery process can be continued.
\newline
