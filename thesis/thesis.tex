...
TODO: ADD INTRO TEXT: Why is LTA interesting and relevant for agriculture?\newline

Increasing efficiency in agriculture is not only sensible, but perhaps even necessary.
The rapidly growing human population and environmental changes require a significant increase in production to meet rising demand. \cite{Virlet:2016}\cite{KhakPour:2021}
\cite{Roure:2018}\newline

The idea that robots can be used to efficiently monitor large areas has numerous real-world applications in precision agriculture. \cite{Bargoti:2015}\newline

However, it is not the case that robots can merely improve the efficiency in tasks that could have been done by humans. There are examples of tasks that only become possible
through the use of robotic systems: \textquote{A robot's \textquote{eye} is far better than human one, and it can collect a large amount of data that are invisible
for us.} \cite{Ampatzidis:2017}\newline

Since the beginning of the 21st century, numerous robotic applications for agriculture have been developed to increase productivity and improve the accuracy, efficiency and
safety of agricultural production processes. \cite{Xue:2010}

Autonomous crop monitoring, a classic domain of precision agriculture, has the goal of detecting potential problems as soon as possible in order to avoid yield losses
while optimizing costs (economic efficiency) and environmental impact. \cite{Dong:2014} \cite{Dong:2017}

The ultimate goal of precision agriculture is to automatically obtain relevant crop health information to detect and treat biotic and abiotic stresses to prevent yield loss 
without the need for manual sampling, which is too expensive and inefficient. \cite{Carlone:2015}

Long-term autonomous robotic systems in highly dynamic real-world applications are becoming a reality. \cite{Kunze:2018}\newline

Achieving robust, long-term autonomy is particularly challenging because it is impossible to envision all potential situations that the robot may encounter.
\cite{Hawes:2017}\newline
A particular challenge is the navigation in such scenarios in highly dynamic real-world environments. \cite{DelDuchetto:2018}\newline

While long-term autnomous robots have enormous potential to increase efficiency and reduce the burden on humans in many areas, they are often not yet reliable
and safe enough to be used with a clear conscience in a wide variety of real-world environments. \cite{Arvin:2018}

Execution monitoring approaches that address failures preventing a system's long-term autonomy by providing recovery mechanisms have to be a crucial component
of long-term autonomous architectures. \cite{Kunze:Kunze:2018}\newline
...
necessarily monitor them closely, but only provide assistance when needed. \cite{Rosenthal:2011}\newline

According to Pinillos et al. there are two major requirements that robots must meet in order to be commercially successful: A reasonable price-performance ratio
and a robust task performance. \cite{Pinillos:2016} This thesis is primarily concerned with the latter. Almost regardless of the actual service a robot provides, it must be robust
in its basic autonomous capabilities such as localization, navigation, perception, etc. Pinillos et al. underline another aspect that is a major motivation for this work:
\textquote{Research efforts often focus on robot localization, navigation, planning or face recognition, but fail in the integration of different technologies to create useful
applications.} \cite{Pinillos:2016} This is precisely the objective of this thesis - to work towards robust integrated systems.\newline

As Ingrand at al. put it: \textquote{Autonomous robots facing a diversity of environments, a variety of tasks and a range of interactions cannot be
preprogrammed by foreseeing at the design stage all possible courses of actions they may require. These robots need to perform some explicit deliberation.} \cite{Ingrand:2017}
They highlight that deliberation capabilities increase the flexibility and robustness of robotic systems.
Interestingly, the deployment of autonomous robots in agricultural scenarios is one of the most challenging domains in the field.
Although (semi-)autonomous planetary exploration rovers such as \textit{Curiosity} encounter manifold and demanding tasks, the environment is quite predictable and exhibits
...
For the authors, the attribute "long-term" implies that the robot should be able to run continuously and autonomously in its environment for at least several weeks, which requires
a certain resilience. They note that this robustness can be achieved both at design time and at runtime.\newline
...
unmanned aerial vehicles (UAVs) for extended periods of time. AUVs are particularly used for scientific observations and security applications. \cite{Chrpa:2015}
...
autonomously as long as they are confronted with a problem they cannot solve. In which case: \textquote{[...] critical aspect of autonomy in our unsupervised application is the
ability to detect failure and signal humans for help.} \cite{Nourbakhsh:2003} The authors have already implemented some diagnostics and ideas for responding to failure cases,
...
\newline
A very recent example of an LTA mobile robot serving as a tour guide in a museum is provided by Del Duchetto et al. \cite{Duchetto:2019}, who present their robot
...
Furthermore, Mühlfellner et al. \cite{Mühlfellner:2015} present an approach for lifelong visual localization and mapping based on three steps:
\begin{enumerate}
...
\end{enumerate}
The authors have an interesting facet to their approach: \textquote{[...] we are concerned not so much with maintaining an accurate model of the world, but rather with maintaining
a useful model.} \cite{Mühlfellner:2015}\newline
...
\textquote{[...] if these disturbances are not addressed, the possibility of failure becomes almost assured if the robots are required to operate over long time horizons.}
...
In conclusion, there is not a great deal of research on long-term experiments with autonomous robots, mainly for two reasons:
...
And further: \textquote{[...] the performance of robotic systems cannot be optimal in real-world situations which are impacted by the robustness of the deployed systems}.
...
employd submodules, which is great motivation for this thesis.\newline
...
follows rows of trees in an orchard based on laser data. One of the most important insights, which may also be relevant to this work, is that it is crucial to keep the project
consistent with the actual requirements of the intended users and real-world applications. Only then can the result actually be of use to practitioners. In addition, data collection
in an agricultural sector almost always requires georeferencing of the data.\newline
...
stay functional for extended periods of time. \cite{Kunze:2018} In contrast, a highly dynamic, partially unknown environment such as in this scenario poses much more difficult
...
\cite{Kortenkamp:2009} There is even evidence that \textquote{Human-Initiative (HI)} systems that can switch between different autonomy levels during operation perform better than
...
i.e. are connected to the GNSS service. Bargoti et al. \cite{Bargoti:2015} identify occlusions of GPS satellites leading to jumps in estimated position as one of the main reasons
...
The authors define the errors as follows: \textquote{[...] situation in which the robot is not able to progress toward the goal, because it is not
...
drastic weather changes that force the robot to interrupt its task. As Harris et al. remark: \textquote{Fixed plans may fail if states encountered during execution do not
...
proprioceptive as well exteroceptive sensors and information. As Hawes et al. put it: \textquote{A key strategy for
...
the interest of safety. \cite{Furgale:2015} Ingrand et al. perfectly summarize the view of monitoring in this work: \textquote{Compares what is predicted regarding the robot
...
\textquote{Plans must be executed according to some strategy. This must account for action failure, plan failure due to ignorance or change in a dynamic environment [...].}
\cite{Cashmore:2015}
...
estimation of one or more of these components. An implementation of a generalized
...
weaknesses cancel each other out, which is why they complement each other well. Odometry, for instance, is very good at continuously approximating the distance traveled,
...
history is tracked. The length of the history $n$ is user-configurable, the default is $n = 1500$. The first aspect monitored is the angular velocity for passive IMU data $a^p$ in $rad/s$.
...
The next aspect is the linear acceleration in $m/s^2$. In preparation, a user-configurable fraction of the active and passive IMU
...
The problem can be trivially simulated by simply not launching the plan generation service. TODO...
...
Requesting assistance from a human operator was identified early on as a crucial aspect of long-term autonomous mobile robots: \textquote{[...] critical aspect of autonomy in our
...
(autonomy percentage), on the other hand, is very useful for this work as well. \textquote{The motivation of the autonomy percentage is that it is of little value to achieve a long
