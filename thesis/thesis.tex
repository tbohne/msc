...
considered together under the heading \textit{Navigation Failure}. Although sustainable recoveries could in principle refer to all kinds of problems, this thesis is limited to
sustainable recoveries in the case of navigation problems, as this is by far the most relevant case from a practical perspective. Thus, it is sustained recoveries initiated by
...
is a path to the navigation goal, but \code{move\_base\_flex} cannot find it (cf. fig. \ref{} TODO). Despite the fact that such examples seem somewhat contrived, it is not
...
them. In the second case, however, it should be detected and communicated in any case, because the robot can not solve it itself. If the robot is unable to successfully generate a
...
behaviors and decide whether it is appropriate to try more or stop. To identify sustained recovery situations, it would be necessary to count them somehow. \code{move\_base\_flex}
...
from \code{GoalStatus.ACTIVE} to \code{GoalStatus.ABORTED}. Then one can stop after $10$ failed recoveries, for example. \code{GoalStatus.SUCCEEDED} should always reset the counter,
because then the recovery has actually solved the problem. This method covers the case of a sustained recovery, but also obstacles without a circumvention option that can lead to a
sustained recovery. Combined with a check for explicit \code{move\_base\_flex} errors, this covers all navigation failures, since they will all end with a persistent recovery or an
explicit termination. One problem is that repeated recoveries in which some progress is actually achieved should not be mistaken as sustained recoveries and thus failure. Hence, one
should not only count the repetitions and declare that it has failed after a number of unsuccessful attempts, but also take into account reasonable progress towards the goal. For
instance, if the robot approaches the target during the process rather than just rotating on the spot, it should be viable.\newline
Sustained recoveries as well as explicit \code{move\_bas \_flex} failures can be evaluated in simulation by inducing situations where recovery is not possible, e.g., when the
...
with obstacles systematically, the \code{ObstacleSpawner} was developed. The extreme case is simulated by spawning a \textquote{robot prison}. Any navigation target outside the prison
leads to an error situation, which should be recognized. The idea is to dynamically spawn obstacles around the robot's current location encircling it. Since spawning the
\textquote{robot prison} takes a moment, it is advisable to do this only when the robot is standing still, otherwise unwanted side effects such as collisions may occur. Consequently,
when the simulation is launched, it will only be executed when the robot is next at a standstill. The \textquote{robot prison} exemplifies cases that cannot be solved by the robot
and that should be detected by monitoring. Furthermore, the following solvable static obstacle scenarios were implemented in order to test the robot's ability to overcome them.
[TODO: add figures.] In this manner, the situation that a static obstacle appears on the robot's route can be simulated, and it is usually able to manage it (cf. fig. \ref{} TODO).
This illustrates that unexpectedly appearing obstacles are not a great problem as long as there is a way. Nevertheless, if they are, it will be detected by the aforementioned
monitoring method. The models used must be stored in \code{~/.gazebo/models} and are all freely available \footnote{http://models.gazebosim.org/}. Unfortunately, it is not trivial to
...
simulate such situations works as follows. The user can specify a set of points, e.g., the one shown in fig. \ref{} in the scenario described in Section \ref{}, selecting the point
farthest from the robot's current position, as this will most likely cause navigation problems. [TODO: add points in crop layout] As anticipated, this is not a guarantee of navigation
failure, but it does cause it on a regular basis. It would be infeasible to simply interrupt the active plan and set the selected point as navigation target, as there would then be
no successful outcome that continues the plan. Thus, the dummy target is introduced as an intermediate action in the plan (an option implemented as part of the low-level operation
state machine).
