...
The classification of the selected LTA problems is based on their evaluation in the simulation. The very first step is to show that, without simulating any of the identified LTA
...
monitoring of these phenomena is necessary. However, drastic weather changes per se will never lead to a \code{CATASTROPHE}, since the robot always has the option to autonomously
return to its base and seek shelter. Naturally, if the robot gets damaged, catastrophic situations could occur, but they would no longer fall under the category of drastic weather
changes. Analogously, without the active monitoring, errors in sensor perception and data management would simply be disregarded and go unnoticed. The robot's mission is not affected,
...
completely discharged battery, whichever comes first, and thus \code{CATASTROPHE}. This again holds for simulation as well as for practice. That localization errors can have dramatic
consequences up to \code{CATASTROPHE} cases has already been shown by the example of GNSS connection issues.
