...
Since the identified challenges of \textit{Obstacles Blocking the Planned Path} and \textit{Sustained Recovery} are also essentially navigation problems,
the three challenges are considered together under the heading of \textit{Navigation Failure}. Although in principle sustained recoveries could refer to all kinds of issues,
for this thesis it is limited to sustained recoveries in the case of navigation issues as this is by far the most relevant case from a practical perspective. Thus, we are
talking of sustained recoveries initiated by \code{move\_base\_flex}.\newline
...
Static obstacles, on the other hand, may be present for longer  periods of time, requiring more sophisticated recovery behaviors. \code{move\_base\_flex} is generally already
able to detect obstacles. The task of a monitoring procedure in this case is to closely observe what \code{move\_base\_flex} does and intervene if necessary.
Regarding obstacles, there are two distinct cases: Either the obstacles are only blocking the direct way, but there still is one. Or, the obstacles are actually blocking every
single way and the robot can't reach it's goal. In the first case, it would be best if the robot would be able to find it, but if it doesn't, it should be communicated.
This is basically the case of navigation failures in the sense that there is a way to the navigation goal, but \code{move\_base\_flex} fails to find it (cf. fig. \ref{}).
Although such examples are somewhat constructed, it is not unrealistic for the robot to face such situations in practice from time to time, i.e. it is good to recognize them
and thus enable the system to deal with such situations. To overcome the problems, one could develop specific recovery behaviors for the considered scenario, analogous to
Duchetto et al. In the second, however, it should be detected and communicated in any case as the robot can't solve it by itself.
There are essentially two different problem cases in which it is not able to find a way, no matter whether this is because there is none or because it is just unable to find it.
First, it ends up in a sustained recovery. Second, it just fails with an error and aborts the navigation target. Both cases should be detected.
During practical experiments within the scope of this work, there were many situations in which \code{move\_base\_flex} was not able to find a correct way and simply ended up in
a loop of failing recoveries without ever getting back to normal operation. In this case, the robot should try a few different recovery actions and then abort and report the
problem. Monitoring methods must therefore track the results / states of recovery behaviors and decide whether it is appropriate to try more or stop.
In order to detect sustained recovery situations, it would be necessary to somehow count them. \code{move\_base\_flex} repeatedly sends \code{GoalStatus.ABORTED} on
\code{move\_base\_flex/exe\_path/status}. To count the number of executed recoveries one can simply count the transitions from \code{GoalStatus.ACTIVE} to \code{GoalStatus.ABORTED}.
Then one can stop after  e.g. 3 recoveries. \code{GoalStatus.SUCCEEDED} should always reset the counter because then the recovery actually solved the problem.
This method covers the case of a sustained recovery, but also obstacles without a way around, which sort of implicitly contains a sustained recovery.
Essentially, this covers all the navigation fail issues as they will all end in a sustained recovery or explicit abortion.
One issue is that repeated recoveries that indeed achieve some progress should not falsely be de detected as sustained recoveries and thus as failure.
Therefore, one shouldn't only count recoveries and say that it failed after a number of attempts without success, instead one should also consider reasonable progress towards
the goal. If during the process the robot got a lot closer to the goal (not only rotating on spot), it should be ok.\newline
Sustained recoveries as well as explicit failures by \code{move\_base\_flex} can be evaluated in the simulation by inducing situations where recovery is not possible, such as when
the path to the destination is blocked and there is no other way, i.e. through specially constructed examples that are particularly difficult for the navigation algorithms to solve.
The occurrence of static and dynamic obstacles can be implemented in the simulation by spawning mobile and immobile
objects at randomized positions that could potentially block the robot's path. However, as mentioned, dynamic obstacles are generally not that much of a problem as they will appear
as obstacles, initiate a recovery, usually a rotation on spot and afterwards it will in most cases already be gone and the robot can continue its mission. Therefore, only the harder
case of unexpected static obstacles is considered here, because this is the case, the robot actually has to adapt to. The most extreme case of an unexpected static obstacle would
be one that blocks every way between the robot and its navigation goal such that it simply cannot reach it. The \code{obstacle\_spawner} was developed to simulate certain scenarios
with obstacles. This case is simulated by a \textquote{robot prison}. Every navigation goal outside of the prison will cause such a failure situation that should be detected.
The idea is to dynamically spawn obstacles around the robot's current location, encircling it. As the spawning of the \textquote{robot prison} takes a moment, it is reasonable to only
do that when the robot is standing still, otherwise the robot could smash into the wall. So, whenever the simulation is initiated, it will only be executed when the robot is
standing still the next time. The following static obstacle scenarios have been implemented in order to test the robot's ability to deal with them: [TODO: add figures.]
Thus, we can simulate the situation of a static obstacle appearing on the robot's route and it is able to deal with this (cf. fig. \ref{}). This highlights that appearing obstacles
are not much of a problem as long as there is a way. Additionally, the \textquote{robot prison} exemplifies cases that cannot be solved by the robot, which should be detected by
monitoring. The obstacle scenarios are implemented in a way that allows to spawn them systematically from code. The models that are going to be spawned have to be stored in
\code{~/.gazebo/models}. All of the used models are freely available at \code{http://models.gazebosim.org/}.
Unfortunately, it is not trivial to setup a situation that causes \code{move\_base\_flex} to fail in each attempt although there is a way. Based on the specific location and
surroundings of the robot, sometimes it is able to find the correct way after a while. Nevertheless, it suffices to show that such situations exist as that already sufficiently
motivates the necessity of dealing with them. The simple approach to simulate such situations works like this: The user can define a number of points, e.g. the following in the
scenario described in section \ref{}: [TODO: add points in crop layout] and the one farthest away from the robot's current location is choosen as this will most likely cause
navigation problems. That shoudl trigger a navigation failure -- not guaranteed, but in many cases. As it won't be good to interrupt the plan and set one of these points as nav
goal (then we'd have no success option -- the plan should basically seeminglessly continue after the "nav challenge"). Thus, the far off driving goal gets introduced as next action
into the plan, which was implemented as part of the low-level operation state machine.
