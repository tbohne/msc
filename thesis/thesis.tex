...
\section{High-Level State Machine Architecture}

The plan execution as well as the monitoring of the robot's operation are modeled as a high-level hierarchical state machine, whose architecture is shown
in figure \ref{fig:high_level_smach}.
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{pics/high_level_smach.jpg}
    \caption{\textsc{Architecture of the Hierarchical State Machine}}
    \label{fig:high_level_smach}
\end{figure}
As depicted in figure \ref{fig:high_level_smach}, the robot starts in the state \code{normal\_operation} in the high-level hierarchical state machine, 
which is represented by an embedded state machine called \code{OperationStateMachine}. During the state \code{normal\_operation}, i.e. in the nested state machine
\code{OperationStateMachine}, the robot can be either in the state \code{idle}, in which it is waiting for a plan, or in \code{execute\_plan}, in which it is executing a given plan.
\code{OperationStateMachine} has three possible outcomes - \code{contingency}, \code{catastrophe}, and \code{shutdown}.
It starts in the \code{idle} state and remains there as long as no plan is provided via a ROS service.
There are two options for the robot to leave the state. The first is via a message on a ROS topic indicating that the end of the current long-term episode
has been reached, resulting in an overall outcome of \code{shutdown} for \code{OperationStateMachine}. The second is to receive a plan that results in a state
transition to \code{execute\_plan}. There are four possible outcomes here, two of which result in state transitions and two of which cause \code{OperationStateMachine} 
to return to the parent state machine. The first possible outcome is \code{action\_complete}, which indicates that an action of the currently processed plan was 
...
The next possible outcome is \code{plan\_complete}, which means that the entire plan has been successfully processed and causes a transition back to the \code{idle} state.
These were the possible outcomes that cause a transition to another state. Now there are the two remaining outcomes that cause a return to the parent state machine. 
...
Finally, there is the potential outcome \code{hard\_failure}, which represents situations where the robot is unable to solve the problem itself.
Now that we have seen what happens in \code{normal\_operation}, we come back to the parent state machine. 
As mentioned, when \code{OperationStateMachine} returns \code{shutdown}, the parent state machine transitions to the \code{shutdown} state and the 
robot's work for that long-term episode is complete. Essentially, in a long-term episode in the considered scenario, the robot should stay in \code{normal\_operation} as 
...
intervention of a human operator. Accordingly, if \code{normal\_operation}, i.e. \code{OperationStateMachine} has the outcome \code{soft\_failure}, the robot enters the
state \code{contingency}, which means that it has detected a minor problem, e.g. a sensor failure, but its ability to act is preserved, it is for example able to drive back 
to the base in safety mode. However, if \code{normal\_operation} returns \code{hard\_failure}, there is a critical problem that is unsolvable for the robot, perhaps even
for the operator, and the robot transitions to \code{catastrophe}. If it is still possible, the robot saves the current state of the plan execution, sends an emergency signal,
e.g. communicates the problem to the operator, and shuts down, i.e. transitions to \code{shutdown}.
...
Besides, there are two more transitions: From \code{contingency} back to \code{normal\_operation}, after a minor problem is resolved by the robot, it can continue
