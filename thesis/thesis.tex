...
The robot's localization is multi-layered and based on sensor fusion of IMU, odometry and RTK-GNSS data using the \code{robot_localization} \cite{Moore:2014} package.
...
For navigation, the ROS package \code{move_base_flex} \cite{Puetz:2018} is used, a more flexible extension of the standard ROS navigation framework \code{move_base}.
...
but in general plans will focus on two types of actions. The first type is \code{drive_to$(lat, lng, \theta)$}, which causes the robot to drive to the specified latitude, 
...
highly dynamic real-world environments. \cite{DelDuchetto:2018} For example, the local planner configured in \code{move_base_flex} (e.g. DWA, EBand) provides a path that
...
detected by \code{move_base_flex}.\newline
...
Let $A := \{$\code{drive_to($lat$, $lng$, $\theta$)}, \code{return_to_base}, \code{charge}, \code{scan}$\}$ be the set of available actions in the considered scenario
...
but \code{drive_to$(lat, lng, \theta)$} needs the additional arguments latitude ($lat$), longitude ($lng$), and orientation ($\theta$) of the target pose. Leaving aside the
...
\ref{sec:docking_solution}), there are two other implicit actions - \code{dock} always follows \code{return_to_base} and \code{undock} follows \code{scan}.
...
Therefore, the \code{drive_to$(lat, lng, \theta)$} and \code{scan} actions are implemented as ROS \code{SimpleActionClient}s that can be interrupted at any time
...
In \code{NORMAL_OPERATION}, the battery discharges according to the plan. There may be some fluctuations due to temperature, for example, but it works roughly as expected.
...
of the battery. More specifically, the current state of charge is constantly kept track of via \code{/arox/battery_param}. In case a charge action is performed, which is communicated
...
types of position covariances. First, \code{COVARIANCE_TYPE_UNKNOWN} if the GNSS receiver does not yield a quality estimate and the GNSS data should be treated with caution.
...
The first type of monitoring for real failures is the status monitoring. If the status is not set to one of the available options (\code{STATUS_NO_FIX}, \code{STATUS_FIX},
