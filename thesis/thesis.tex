...
for many aspects considered in this work: \textquote{\textit{[...] our primary concern is not autonomy for its own sake, but rather making robots do useful work, then we
...
fine to seek assistance from human operators from time to time to increase the robustness of the system. As a result of this work, they have achieved a level of robustness that
...
robots like web servers that still need human administration in case of errors. Two critical requirements are cited for the long-term autonomy of a mobile robot in an office
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{pics/autonomy_robustness.png}
...
\cite{Meeussen:2011} For the authors, this is another motivation for including a human operator as a fallback solution. Their key insight for this thesis is the importance of having
...
Brommer et al. \cite{Brommer:2018} are studying long-term autonomy of UAVs, which are also commonly used for monitoring purposes in precision agriculture. One problem with using such
...
landing. They also introduce a system state monitoring system that keeps track of critical aspects such as unexpected low battery and triggers a response when needed.\newline
Kunze et al. \cite{KunzeSI:2018} discuss challenges for long-term autonomous robots in real-world environments and how these can be addressed with techniques from the field of AI.
First, they note that such systems must be both hardware and software robust, meaning that the system must be able to cope with any potential environmental changes expected
during its deployments. \cite{KunzeSI:2018} They point out that much of the long-term autonomy research focuses on coping with changing environments. The work further suggests that
...
inherent communication delays. Due to this delay, teleoperation is generally infeasible for planetary rover applications, and a certain level of autonomy is mandatory for successful
...
consider mandatory for any system operating in an unknown environment. Thus, they rely on shared-autonomy whenever necessary. The work describes various recovery methods for failure
...
Visual place recognition in changing environements is a very active area of research, where it is far beyond the scope of this thesis to cover all recent advances (cf. \cite{Porav:2018},
\cite{Lowry:2016}, \cite{Suenderhauf:2013}, \cite{Lowry:2016}, \cite{Milford:2012}, \cite{Churchill:2013}). In the scenario considered in this work, it is particularly relevant that the robot is able to recognize the place of the mobile container
