...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{pics/questionnaire.png}
...
\begin{figure}[H]
    ...
    \begin{tabular}{| c | c | c | c |}
        \hline
        \textbf{problem} & \textbf{\o \thinspace impact} & \textbf{\o \thinspace difficulty} & \textbf{\o \thinspace likelihood} \\ \hline
        power\_management &  &  &  \\ \hline
        charging\_failure &  &  &  \\ \hline
        drastic\_weather\_change &  &  &  \\ \hline
        certain\_dynamics &  &  &  \\ \hline
        sensor\_failure &  &  &  \\ \hline
        perceptual\_aliasing\_issue &  &  &  \\ \hline
        data\_management &  &  &  \\ \hline
        lost\_connection &  &  &  \\ \hline
        obstacles\_blocking\_path &  &  &  \\ \hline
        robot\_gets\_stuck &  &  &  \\ \hline
        robot\_falls\_over &  &  &  \\ \hline
        navigation\_failure &  &  &  \\ \hline
        sustained\_recovery &  &  &  \\ \hline
        incorrect\_localization &  &  &  \\ \hline
        mapping\_error &  &  &  \\ \hline
        plan\_deployment\_failure &  &  &  \\ \hline
    \end{tabular}}
    \caption{\textsc{Average Results of the Evaluation}}
...
described in section \ref{sec:dummy_scanning_node}. However, there are more subtle failures than just a total breakdown. One issue could be an empty list of range values,
i.e. a scan arrives on the topic but is essentially vacuous. As mentioned in section \ref{sec:dummy_scanning_node}, the crucial part of a laser scan is the list of range values,
which makes it useless if this list is empty for some reason. It is again rather trivial to simulate such a case by clearing the list of range values before republishing
a scan in the dummy scanning node. Another, more subtle, problem could be realized by publishing a scan that contains only maximum range values, i.e. \code{inf}, which may
indicate that the robot has fallen over or that the sensor has slipped out of position. In any case, this could indicate that the sensor is pointing towards the sky.
Finally, it could be a problem if the same scan is published repeatedly. It is possible that the range values between two scans where the robot has not moved are similar,
although they should never be the same due to noise, etc. However, what should be different in each case is the metadata such as the ID.
To simulate repeated scans, the dummy scanning node always stores the previous scan and can replace the current scan with the previous one on command.
...
\begin{itemize}
    \item \textbf{total sensor failure} \textrightarrow \code{/toggle\_simulated\_sensor\_failure}
    \item \textbf{empty list of range values} \textrightarrow \code{/toggle\_simulated\_empty\_ranges}
    \item \textbf{look to the sky} \textrightarrow \code{/toggle\_simulated\_look\_to\_sky}
...
For this purpose, the incoming scans must be examined more closely. For example, if the list of range values is empty, it is as useless for the mission as a scan that
does not arrive at all. Hence, after the arrival of a new scan, the monitoring solution verifies that the list of range values is not empty.
Another interesting issue to detect is a list of range values that only contains \code{inf} values, which, as mentioned in section
\ref{sec:simulation_of_lta_challenges}, could indicate that the sensor is pointing towards the sky for some reason, e.g. because the robot has fallen over,
...
reasonable-looking values when the robot is tilted on its back and the sensor is pointing toward the sky, as shown in figure \ref{fig:tilted_AROX}.
