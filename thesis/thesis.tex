...
\begin{itemize}
    ...
    \item \textbf{sensor data processing failure}
    \item \textbf{lost connection} (e.g. WiFi, RTK, UWB, LTE)
    \item \textbf{incorrect / inaccurate localization} (GPS, IMU, odometry)
    \item \textbf{certain dynamics} (e.g. day / night)
    \item \textbf{data management} (e.g. full memory)
    \item \textbf{sustained recovery}, i.e. no return to normal operation
    \item \textbf{mapping issues} (e.g. incorrect costmap entries)
    \item \textbf{plan deployment failure}, i.e. robot remains in \code{IDLE} state
\end{itemize}
...
\noindent
i.e. unsuccessful docking with inductive charging station / charging process does not start after docking\newline
TBD...\newline
...
\noindent
\textbf{Sensor Data Processing Failure}\newline
