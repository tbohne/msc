...
with obstacles systematically, the \code{ObstacleSpawner} was developed. The models used must be stored in \code{\textasciitilde/.gazebo/models} and are all freely available
\footnote{http://models.gazebosim.org/}.
The extreme case is simulated by spawning a \textquote{robot prison}. Any navigation target outside the prison
...
intermediate action in the plan (an option implemented as part of the low-level operation state machine).
