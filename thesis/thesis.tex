...
their diagonal, since the diagonal contains the covariance of each variable with itself, which is equal to the variance of that variable. \code{NavSatFix} distinguishes different
...
to take the square root of the diagonal values to obtain the more meaningful standard deviations. Eventually, \code{COVARIANCE\_TYPE\_KNOWN} means that the GNSS receiver provides the
complete covariance matrix of $3 \times 3$ values and the GNSS data can be trusted as the error estimation is quite sophisticated. The covariances are not directly based on the
...
plane touches the Earth's surface. The two-dimensional \textquote{East-North-plane}, together with the \textquote{Up-axis} perpendicular to the Earth at the reference point, form
...
\end{figure}
\noindent
The position covariances in a \code{NavSatFix} message are defined in square meters relative to the tangential plane, i.e. a $3 \times 3$ matrix of ENU components. Thus, one has to
take the square root of the values on the diagonal of the matrix to end up with the actual deviation in meters, i.e. how widely the GNSS values are currently spread in meters. In
general, a minor deviation implies that the uncertainty about the current estimated position is very small, making the estimate very accurate.
...
\code{COVARIANCE\_TYPE\_DIAGONAL\_KNOWN} (diagonal known or entirely known), and each deviation, i.e. the square root of the values on the diagonal of the
covariance matrix, is $\leq d_{max} \in \mathbb{R}_{0}^{+}$, which is an upper bound of what is considered to be a reasonable deviation by the user. The last entry of the matrix, i.e. the
variance of the \textquote{Up-component}, may not always be present, so \code{float(`nan')} is also accepted here. It is
classified as medium if the status is $\geq$ \code{STATUS\_FIX}, the covariance type is $\geq$ \code{COVARIANCE\_TYPE\_APPROXIMATED}, and the deviations in turn satisfy the upper
limit of reasonable deviations specified by the user. Finally, it is classified as low if it provides a position belief state ($\geq$ \code{STATUS\_FIX}), but the covariance type
...
precisely, the deviations, i.e. the square root of the diagonal values, because they actually provide information about the accuracy of the estimated position.
As for the actual deviation values, two things can be observed: The absolute values (appropriate / too high) and the progression over time.
...
history, the square roots, i.e. the deviations, are considered, since, as mentioned, these are the crucial values provided by most GNSS receivers.
...
messages, the deviation progression is analyzed.
Such an analysis primarily examines the deviations of the east and north components, but also those of the upward component, if present. First, it is checked whether the history
contains only increasing values for the respective component. If this is the case and, in addition, the difference between the oldest and the newest deviation is greater than a
user-defined significant deviation increase $d_{inc} \in \mathbb{R}_{0}^{+}$, a contingency is initiated. Thus, the definition of a bad deviation progression is exclusively
increasing values together with an overall absolute increase between the oldest and latest value that exceeds a certain threshold.
...
$c_{ii} \in \mathbb{R}_{0}^{+}$ can with a clear conscience be used for quality assessment. If any of the values exceeds a user-specified maximum deviation threshold,
i.e. $\exists c_{ii} : \sqrt{c_{ii}} > d_{max}$, normal operation is interrupted by publishing to \code{/contingency\_preemption}. 
...
The deviation values $\sqrt{c_{ii}}$ are checked, but high deviations exceeding the threshold do not cause a contingency, only a message on \code{/robot\_info}.
...
reciprocal relationship of the directions under consideration. For instance, if $c_{NE} < 0$, a negative error in the north direction is likely to be matched by a positive error in
...
contains increasing deviation values for the east component ($\sqrt{16.2} m \rightarrow \sqrt{25.1} m \rightarrow \sqrt{89.0} m \rightarrow \sqrt{99.4} m$) and the overall
...
of the connection monitoring described in section \ref{sec:}. In contrast, IMU and odometry data are not as straightforward to
...
(cf. section \ref{sec:}), a high accuracy is assumed for this component because if this were not the case, it would already have been detected by the corresponding connection
