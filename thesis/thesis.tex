...
\begin{itemize}
    ...
    \item robot gets stuck (e.g. spinning wheels due to a muddy path)
    \item navigation failures (e.g. global plan not feasible)
    \item hardware issues (e.g. sensor failures)
    \item lost connection (e.g. WiFi)
    \item incorrect / inaccurate localization
...
\end{itemize}
For each potential problem identified, its relevance is briefly explained below, along with why it might be problematic.
The idea is to have monitoring nodes that look for the specific types of faults and report them via the \code{MonitorState}s topic,
which triggers a transition to \code{CONTINGENCY} or \code{CATASTROPHE}. Via the published message, the information about the required
method of resolution is transmitted, depending on the type of fault. Therefore, the overall goal is to have a resolver node for each of the 
potential failures that is executed in \code{CONTINGENCY} or \code{CATASTROPHE} based on the failure case at hand.
...
\noindent
\textbf{Hardware Issues - Sensor Failures}\newline
...
as it can be easily simulated by stopping to publish laser scans in the fake scanner node. In such a case, a monitoring procedure should be able to detect this,
