...
position. There are several local tangent plane systems, the specific one assumed in \code{NavSatFix} is the \textquote{East-North-Up} (ENU) coordinate system. To give an example,
let the locations considered be the \textit{Innovation Center Osnabrueck (ICO)} and the \textit{DFKI-Labor Niedersachsen}, shown in fig. \ref{fig:ICO_DFKI_map}. A typical GNSS
receiver would estimate positions in latitude (degrees), longitude (degrees), and altitude (meters), e.g. \textit{ICO} ($52.287690, 8.018690, 63.0$) and \textit{DFKI}
($52.287863, 8.027347, 63.0$). Let the ENU representation be constructed with respect to the \textit{ICO} location, i.e. this location will be the origin of the tangential plane.
Now another location, for instance, that of the \textit{DFKI}, is considered. The ENU provides the information where this point is located in the local coordinate system in meters.
The result ($590.7, 19.3, 0.0$) is displayed in fig. \ref{fig:ICO_DFKI_ENU}, i.e. the DFKI is located $590.7 m$ east and $19.3 m$ north of the ICO site. In this case, both locations
...
\end{figure}
\noindent
The position covariances are defined in square meters relative to that plane, i.e. a $3 \times 3$ matrix. In general, a low variance means that the region
in which the current position is estimated is very small, so the estimate is very precise, i.e. there are no jumps with large distances.\newline
