...
Plan execution as well as robot operation monitoring are modeled as a high-level hierarchical state machine, whose architecture is shown
in figure \ref{fig:high_level_smach}.
\begin{figure}[H]
    \centering
    \includegraphics[width=0.65\textwidth]{pics/SMACH_complete.png}
...
\code{NORMAL\_OPERATION} is represented by an embedded state machine with the same name that is visualized in figure \ref{fig:low_level_smach}.
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{pics/SMACH_low_level.png}
    \caption{\textsc{Architecture of the Embedded State Machine}}
    \label{fig:low_level_smach}
\end{figure}
\noindent
During the state \code{NORMAL\_OPERATION}, i.e. in the nested state machine, the robot can be either in the state \code{IDLE}, in which it is waiting for a plan, 
or in \code{EXECUTE\_PLAN}, in which it is executing a given plan. The embedded state machine \code{NORMAL\_OPERATION} has three possible outcomes - \code{end\_of\_episode},
\code{minor\_complication}, and \code{critical\_complication}. It starts in the \code{IDLE} state and remains there as long as no plan is provided via a ROS service.
...
transition to \code{EXECUTE\_PLAN}. There are four possible outcomes here, two of which result in state transitions and two of which cause \code{NORMAL\_OPERATION} 
...
Now that we have seen what happens in \code{NORMAL\_OPERATION}, we come back to the parent state machine depicted in figure \ref{fig:high_level_smach}.
...
intervention of a human operator. Accordingly, if \code{NORMAL\_OPERATION}, i.e. the embedded state machine has the outcome \code{minor\_complication}, the robot enters the
...
to the base in safety mode. However, if \code{NORMAL\_OPERATION} returns \code{critical\_complication}, there is a critical problem that is unsolvable for the robot, perhaps even
