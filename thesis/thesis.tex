...
Another monitoring approach that compares different localization components is $yaw$ angle monitoring, arguably the most relevant kind of orientation for ground vehicles.
...
changing the acceleration in $x$- or $y$-direction to a relatively high value (e.g. $8 m/s^2$) so that the robot drifts off. The third option, a divergence between the estimated yaw
angles of the interpolated GNSS positions, the filtered odometry and the IMU, can be simulated by rotating the robot by $180\degree$ around the yaw axis. This assumes knowledge of
the robot's current pose (position + orientation). The position is taken from \code{NavSatFix} messages to the \code{/fix} topic, and the orientation is extracted from the latest
\code{Imu} message to \code{/imu\_data}. The pose is always kept available in terms of latitude ($lat$), longitude ($lng$), and the yaw angle $yaw$ in degrees. Since the IMU stores
...
$yaw = yaw \cdot \frac{180}{\pi}$ to convert the resulting radians to degrees. For the actual simulation, $yaw$ is increased by $180\degree$ if it does not exceed $180\degree$,
since $yaw \in [-180\degree, 180\degree]$. Otherwise, $yaw = -180 + ((yaw + 180) \mod 180)$. Based on this adjusted $yaw$ angle, a \code{move\_base\_flex} navigation target is
created, which then accounts for the actual $180\degree$ rotation. Such a problem can occur in
practice in various situations, e.g. when the robot unexpectedly slips away and turns. Furthermore, the situation of too high IMU acceleration values can be simulated in situations
...
motion for the odometry can be simulated by publishing a linear twist on \code{cmd\_vel} to let the robot move slightly without active navigation goals. Practically, both cases may
...
changes from a passive state to \code{GoalStatus.ACTIVE}, the simulation is initiated. Likewise, the case of a change of position without wheel revolutions should take place in a
passive state. The simulation of deviations in yaw angle estimation is particularly challenging, as it can only be simulated in \code{GoalStatus.ACTIVE}, since the corresponding
monitoring is disabled in passive states due to rotations on spot, which naturally lead to inaccurate GNSS estimates. Such rotations on spot occur frequently as recovery behaviors.
Therefore, the yaw divergence simulation can only be done precisely during the transition from \code{GoalStatus.ACTIVE} to a passive state, when monitoring is still active and GNSS
interpolation is still expected to be accurate. Moreover, the two remaining cases of motion in passive states indicated by IMU and odometry can obviously only be simualted when no
...
twists to the \code{/cmd\_vel} topic. Afterwards, the localization monitoring node is reinitialized, clearing all previously collected data, etc. As usual, if something goes wrong
during the resolution attempt, the fallback solution of asking the operator for help is executed (cf. section \ref{sec:fallback_solution}).
