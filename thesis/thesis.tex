...
outdoor robots by developing a fully integrated framework capable of overcoming some of the typical
challenges and limitations for such systems.
\vfill
\end{prefacesection}
...
problems, the mission runs error-free for an extended period of time (reference time), e.g., $4$ hours, in simulation. This also verifies that the sample plan is reasonable, i.e.,
...
evaluation purposes, let a failed mission be defined as a mission that exceeds a user-configurable timeout $t = 600s$ without publishing a message to \code{arox/ongoing\_operation},
...
stuck in that task for a time greater than $t$. Obviously, a \code{hard\_failure} in the low-level state machine leading to a \code{CATASTROPHE} state followed by a system
...
\code{hard\_failure}, leading to a transition to \code{CATASTROPHE} and then a system shutdown. Thus, the simulation of power management failure works as expected. That the general
