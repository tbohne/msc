...
in figure \ref{fig:high_level_smach}. The architecture is implemented in SMACH\footnote{ROS-independent Python library to build hierarchical state machines} \cite{smach}.
\begin{figure}[H]
    \centering
...
parallel running execution monitoring states (cf. \cite{monitor_state}) \code{CONTINGENCY\_MONITORING} and \code{CATASTROPHE\_MONITORING}, implemented as a concurrence container \cite{concurrence_container}.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{pics/SMACH_low_level.png}
