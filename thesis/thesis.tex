...
respective impact from \textit{high}, i.e. prevents a successful mission, to \textit{medium} impact, e.g. delays missions or reduces the quality of results, 
to \textit{low}, where it has at most a minor impact on the robot's mission. The next aspect is about evaluating each problem in terms of its individual difficulty 
of solution and recognition. The options were \textit{easy}, meaning simple solutions exist, \textit{medium}, which requires some effort but should be feasible within 
the scope of the work, to \textit{hard}, i.e. it is a rather general problem. Finally, the likelihood of occurrence for each problem should be evaluated based on the
three options \textit{very likely}, which roughly means you have experienced this in practice several times, \textit{occurs}, meaning you have experienced or heard of
this once, and \textit{highly unlikely}, meaning you have never seen or heard of this. The questionnaire was completed by a total of $n$ people. Of course, 
