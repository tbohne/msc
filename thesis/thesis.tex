...
The very first step is to show that without simulating any of the identified LTA problems, the mission runs flawlessly for an extended period of time, e.g., $5$ hours. This also
verifies that the sample plan is reasonable, i.e., that the charge stops are sufficient if everything works as expected. Basically a demonstration that the simulation and the whole
integrated system work in principle in the absence of unexpected problematic events. For evaluation purposes, let a failed mission be defined as a mission that exceeds a
user-configurable timeout $t = 900s$ without publishing a message to \code{arox/ongoing\_operation}, although the previous message contained a value of \code{total\_tasks} that is
greater than one, indicating that the current plan is not completed and thus that the robot has been stuck in that task for a time greater than $t$. Obviously, a \code{severe\_failure}
...
unsurprisingly, the average runtime was $5.03$ hours. In this time, the robot has completed an average of $2.4$ missions and $62.2$ tasks. The robot required an average of $4.3$
charge cycles and the average total distance traveled was $1.03 km$. It is also interesting to look at the mode distribution, i.e. how long the robot was in which mode during the LTA
...
The detailed results can be found in the appendix (cf. \ref{}).\newline
