...
\code{NavSatFix} and thus configurable in the \code{GNSS Simulator}. Covariances are commonly represented as a square matrix $C$ containing the covariance $c_{ij}$ for each pair
...
that variable. \code{NavSatFix} distinguishes different
...
currently spread in meters. In general, a minor standard deviation implies that the uncertainty about the current estimated position is very small, making the estimate very accurate.
