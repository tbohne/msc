...
\code{COVARIANCE\_TYPE\_DIAGONAL\_KNOWN} (diagonal known or entirely known), and each standard deviation, i.e. the square root of the values on the diagonal of the
covariance matrix, is $\leq d_{max} \in \mathbb{R}_{0}^{+}$, which is an upper bound of what is considered to be a reasonable deviation by the user. The last entry of the matrix,
i.e. the variance of the \textquote{Up-component}, may not always be present, so \code{float(`nan')} is also accepted here. It is
classified as medium if the status is $\geq$ \code{STATUS\_FIX}, the covariance type is $\geq$ \code{COVARIANCE\_TYPE\_APPROXIMATED}, and the standard deviations in turn satisfy the
upper limit of reasonable deviations specified by the user. Finally, it is classified as low if it provides a position belief state ($\geq$ \code{STATUS\_FIX}), but the covariance
...
history, the square roots, i.e. the standard deviations, are considered, since, as mentioned, these are the crucial values provided by most GNSS receivers.
...
The deviation values $\sqrt{c_{ii}}$ are checked, but high standard deviations exceeding the threshold do not cause a contingency, only a message on \code{/robot\_info}.
