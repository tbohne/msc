...
This is a classic problem in robotics, how can one scientifically argue that the developed approaches work?
There is going to be a sufficiently meaningful, i.e. realistic, simulation that allows an empirical analysis based on a huge number of samples.\newline
...
in the simulation and it will be shown that the system no longer works, i.e. that these barriers are in fact able to cause failure.
Then the goal is to develop solutions that are able to resolve these problems, such that it works again.
One essential goal will be to increase the number of situations in which the robot can resolve such situations by itself. Moreover, it would be very helpful 
for practical applications to at least be able to recognize such problems and call for the help of a human operator. 
...
For example, we'd need to run a simulation and check every few minutes whether the robot got stuck.
